  ...
  try {
    ...
    if (!business) {
      business = await Business.create({
        name: req.user.businessName || `${req.user.name}'s Business`,
        category: req.user.businessCategory || 'Other',
    ...
    if (!isYouTubeUrl(url)) {
      return res.status(400).json({ message: 'Only YouTube URLs are currently supported' });
    }
    ...
    res.status(500).json({ message: `Server error uploading video: ${error.message}` });
  }
};
